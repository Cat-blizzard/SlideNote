...
def build_env(base_env: dict[str, str] | None, config: StudioConfig) -> dict[str, str]:
    ...
    if config.vision != "off" and config.vision_api_key:
...
def build_slidenote_command(config: StudioConfig) -> list[str]:
    ...
    if config.ocr != "off":
        if config.ocr_api_key:
            cmd.extend(["--ocr-api-key", config.ocr_api_key])
        if config.ocr_secret_key:
            cmd.extend(["--ocr-secret-key", config.ocr_secret_key])
    ...
    if config.vision != "off" or config.figure_crop == "vision" or config.figure_grounding == "vision":
        if config.vision_model:
            cmd.extend(["--vision-model", config.vision_model])
        if config.vision_api_key:
            cmd.extend(["--vision-api-key", config.vision_api_key])
