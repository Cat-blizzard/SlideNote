...
| **4. Guard** | 检查生成结果是否保真、是否好学。 | `coverage.json` / `coverage.md`、`content_guard.json`、required visible coverage 修复、source marker、`quality_report.json`、Review/Exam 学习包检查、实验性 Claude repair 检查。 |
| **5. Export** | 发布最终结果并报告运行过程。 | `notes.toc.md`、`notes.docx`、`notes.pdf`、`notes.tex`、`review.md`、`exam.md`、`exam.json`、`exam.html`、`run_summary.json`、usage/cost 报告、SlideNote Studio GUI、实验性 Claude agent pack 和 `agent-eval` 报告。 |
