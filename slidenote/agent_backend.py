...
from slidenote.build.config import _friendly_build_error
...
def _repair_agent_sections_once(
    *,
    ...
    for result in section_results:
        ...
        if not quiet:
            print(f"Repairing {backend} section: {result.get('title') or section_id}")
        try:
            prompt = _build_agent_repair_prompt(manifest, section, section_file, current_markdown, issues)
            parsed, metadata = runner(prompt)
            markdown = str(parsed["markdown"]).strip()
            _validate_asset_references(markdown, parsed["used_asset_paths"], known_assets)
            markdown = _ensure_covered_markers(markdown, parsed["covered_source_ids"], source_id_to_slide)
...
def _validate_agent_result_payload(payload: dict[str, Any]) -> None:
    ...
    if missing:
        raise AgentBackendError(f"Agent result is missing required field(s): {', '.join(missing)}")
    for name, expected_type in required.items():
        if not isinstance(payload[name], expected_type):
            raise AgentBackendError(f"Agent result field `{name}` must be {expected_type.__name__}.")
    if not all(isinstance(value, str) for value in payload["used_asset_paths"]):
        raise AgentBackendError("Agent result field `used_asset_paths` must contain only strings.")
    if not all(isinstance(value, str) for value in payload["covered_source_ids"]):
        raise AgentBackendError("Agent result field `covered_source_ids` must contain only strings.")


_AGENT_SYSTEM_PROMPT = (
    "You are a careful study-note writer for SlideNote. You receive one section pack "
    "with structured slide content, source ids, and image assets. "
    "Return ONLY one JSON object with exactly these fields: "
    '"markdown" (string), "used_asset_paths" (string[]), "covered_source_ids" (string[]), "warnings" (string[]). '
    "Do not write files. Do not invent image paths; use only assets/ paths listed in the pack. "
    "Keep the exact source marker format: <!-- slidenote-source: p{slide_id}:{comma_separated_ids} -->. "
    "Cover every source id either in visible prose or with a hidden source marker; explain figures in visible text."
)


def _parse_agent_json_text(text: str) -> dict[str, Any]:
    """Parse the SlideNote result JSON from an LLM text response."""
    if not text or not text.strip():
        raise AgentBackendError("Agent backend returned empty output.")
    payload = _parse_json_object_from_text(text)
    _validate_agent_result_payload(payload)
    return payload


def parse_dsh_output(text: str) -> tuple[dict[str, Any], dict[str, Any]]:
    """Parse a DeepSeek backend response into (payload, metadata)."""
    payload = _parse_agent_json_text(text)
    return payload, {"source": "deepseek", "mode": "direct"}


def _run_dsh_command(
    *,
    prompt: str,
    pack_dir: Path,
    provider: str,
        ...
        cache_key = make_cache_key(
            {
                ...
                "provider": provider,
                "model": model or "",
                "schema": AGENT_RESULT_SCHEMA_VERSION,
            }
        )
...
def _build_agent_prompt(manifest: dict[str, Any], section: dict[str, Any], section_file: Path) -> str:
...
def _validate_asset_references(markdown: str, used_asset_paths: list[str], known_assets: set[str]) -> None:
    ...
    if unknown:
        raise AgentBackendError(f"Agent referenced unknown asset path(s): {', '.join(unknown)}")
