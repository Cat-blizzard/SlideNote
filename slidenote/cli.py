...
def _build_parser() -> argparse.ArgumentParser:
    ...
    agent_pack = subparsers.add_parser("agent-pack", help="Build an agent-friendly pack without generating notes (experimental).")
    ...
    agent_run = subparsers.add_parser("agent-run", help="Run an agent pack through a stdout-only agent backend (experimental).")
    ...
    agent_build = subparsers.add_parser("agent-build", help="Build an agent pack, run the agent backend, and produce notes plus coverage (experimental).")
    ...
    agent_eval = subparsers.add_parser("agent-eval", help="Compare baseline build output with the agent build (experimental).")
...
def _add_agent_pack_options(command: argparse.ArgumentParser) -> None:
    ...
    command.add_argument(
        "--vision-provider",
        default="qwen",
        help="Vision provider. Supported image providers include openai, qwen, doubao, gemini, and claude.",
    )
...
def _add_agent_run_options(command: argparse.ArgumentParser, include_out: bool = True, include_quiet: bool = True) -> None:
    ...
    command.add_argument(
        "--dsh-provider",
        default="deepseek",
        help=f"DeepSeek backend provider. Supported: {', '.join(supported_provider_names())}.",
    )
    command.add_argument("--dsh-model", default=None, help="DeepSeek backend model. Defaults to the provider default.")
    command.add_argument("--dsh-api-key", default=None, help="DeepSeek backend API key override.")
    command.add_argument("--dsh-base-url", default=None, help="DeepSeek backend base URL override.")
    command.add_argument("--dsh-max-output-tokens", type=int, default=6000, help="Maximum output tokens per section call.")
    ...
    command.add_argument("--dsh-cache-dir", type=Path, default=None, help="DeepSeek backend cache directory.")
